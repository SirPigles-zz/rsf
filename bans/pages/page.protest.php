...
else
{
	...
	if($Type == 2 && !validate_steam($SteamID))
	{
		$errors .= '* Please type a valid STEAM ID.<br>';
		$validsubmit = false;
	}
	elseif($Type==2)
	{
		$pre = $GLOBALS['db']->Prepare("SELECT bid FROM ".DB_PREFIX."_comms WHERE authid=? AND RemovedBy IS NULL AND type=2;");
		$res = $GLOBALS['db']->Execute($pre,array($SteamID));
		if ($res->RecordCount() == 0)
		{
			$errors .=  '* That Steam ID is not muted/gagged!<br>';
			$validsubmit = false;
		}
		else
		{
