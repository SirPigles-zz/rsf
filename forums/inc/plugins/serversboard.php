...
define("COMPATIBILITY", "1801,1802,1803,1804,1805,1806");
...
{$owner[\'profilelink\']}<br /><span>{$owner[\'usergroup\']}</span>
</td></tr>
</table>
<br />'),
		"sid" 				=> "-2",
			...
			while($server = $db->fetch_array($query)) 
			{
				...
				elseif($data['gq_type'] == 'wet')
				{
					...
					if($server['owner'] != '')
					{
						...
						$owner = get_user(get_user_by_username(trim($server['owner'])));
						$owneravatar = format_avatar(htmlspecialchars_uni($owner['avatar']), $owner['avatardimensions']);
						$owner['usergroup'] = $groupscache[$owner['usergroup']]['title'];
...
function serversboard_wol($user_activity)
{
...
function serversboard_friendly_location($array)
{
    ...
    if($array['user_activity']['activity'] == $lang->serversboard_online_location)
    {
        if($array['user_activity']['sid'])
        {
			$sid = $array['user_activity']['sid'];
			...
			$array['location_name'] = $lang->serversboard_online_location_viewing_serversboard_more." <a href=\"serversboard.php?action=more_information&sid=" . $array['user_activity']['sid'] . "\">".$data['gq_hostname']."</a>";
        }
        else
        {
            $array['location_name'] = $lang->serversboard_online_location_viewing_serversboard." <a href=\"serversboard.php\">".$lang->serversboard_online_location_serversboard."</a>";
        }
    }
    return $array;
}
